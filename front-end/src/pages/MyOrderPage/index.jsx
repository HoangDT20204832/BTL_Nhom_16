...
const MyOrderPage = () => {
  ...
  const handleCanceOrder = (order) => {
    mutation.mutate(
      { id: order._id, orderItems: order?.orderItems, userId: user.id },
      {
        onSuccess: () => {
          queryOrder.refetch();
        },
    ...
    return data?.map((order) => {
      return (
        <WrapperHeaderItem key={order?._id}>
          ...
          >
            {order?.priceNew?.toLocaleString()}đ
          </span>
        </WrapperHeaderItem>
      );
    });
  ...
  const handlReceiveOrder = (order) => {
    mutationUpdate.mutate(
      { orderId: order._id, isDelivered: true, isPaid: true },
      {
        onSuccess: () => {
          queryOrder.refetch();
        },
                ...
                <WrapperItemOrder key={order?._id}>
                  ...
                  <WrapperFooterItem>
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      {order?.isDelivered &&
                        order?.isPaid &&
                        !idsOrderReviewed.includes(order?._id) && (
                          <ButtonComponent
                      ...
                      order?.isPaid === false ? (
                        ...
                        <ButtonComponent
                          onClick={() =>
                            navigate(
                              `/product-detail/${order?.orderItems[0]?.product}`
                            )
                          }
                          size={40}
                          styleButton={{
